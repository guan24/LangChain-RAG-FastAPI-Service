...
class DocumentProcessor:
    ...
    def __init__(self, vectors_store: Chroma, md5_store):
        self.vectors_store = vectors_store  # 向量数据库
        self.md5_store = md5_store  # MD5存储管理器
        self.spliter = AsyncTextSplitter(  # 文本分割器
            chunk_size=chroma_config["chunk_size"],  # 分割的块大小
            chunk_overlap=chroma_config["chunk_overlap"],  # 分割块之间的重叠部分大小
            separators=chroma_config["separators"],  # 分割的块的分隔符
            embedding_model=embed_model,  # 用于计算嵌入向量的嵌入模型
        )
    ...
    async def get_document(
        ...
        for idx, file_path in enumerate(file_paths):
            ...
            if await self.md5_store.check_md5_hex(md5_hex, user_id):
                ...
                logger.info(
                    f"【向量数据库】文件 {file_path} 的md5值 {md5_hex} 已存在，跳过"
                )
                if files:
                    try:
                        os.unlink(file_path)
            ...
            try:
                ...
                document: list[Document] = await self.get_file_document(
                    file_path, md5_hex, user_id
                )
                if not document:
                    if progress_callback:
                        await progress_callback(
                ...
                logger.info(
                    f"【向量数据库】开始存储向量: {filename}，文档数量: {len(document)}"
                )
                ...
                original_filename = (
                    file_names.get(file_path, filename) if files else filename
                )
                # 保存 MD5 记录
                await self.md5_store.save_md5_hex(
