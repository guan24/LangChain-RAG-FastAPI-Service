...
# Django JWT配置
SECRET_KEY = os.getenv("SECRET_KEY")  # JWT密钥
ALGORITHM = os.getenv("ALGORITHM")  # JWT算法
...
async def get_current_user_id(
    ...
    if jti:
        ...
        logger.info(
            f"【debug】 检查JWT是否在黑名单中，匹配的键: {matching_keys}",
            extra={"path": "auth_utils.get_current_user_id"},
        )
...
async def fetch_user_info_from_django_api(
    token: str, url: str
    ...
    try:
        ...
        logger.error(
            f"【debug】 调用Django API时出错: {str(e)}",
            extra={"path": "auth_utils.fetch_user_info_from_django_api"},
        )
        return None


async def get_user_info_from_redis(
    user_id: str, credentials: HTTPAuthorizationCredentials
):
    """从Redis中获取用户信息
    ...
    # 调用Django查询用户信息 并 设置Redis缓存
    async def fetch_user_info_from_django_api_And_set_redis_cache():
    ...
    try:
        ...
        if user_info is None:
            # 调用Django查询用户信息并设置Redis缓存
            user_info = await fetch_user_info_from_django_api_And_set_redis_cache()
        else:
